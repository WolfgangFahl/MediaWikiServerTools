...
def _mysql_available() -> bool:
    """Check if any mysql client is reachable (local or via configured servers)."""
    if shutil.which("mysql") or shutil.which("mysqlr"):
        return True
    # check if at least one server with has_sqldb=True is reachable via SSH
    try:
...
class TestSqlBackup(Basetest):
    ...
    @unittest.skipIf(not _mysql_available(), "No mysql/mysqlr available")
    ...
    def check_sqlrestore(
        ...
        # verify data is back
        q = f"SELECT COUNT(*) AS c FROM {db_name}.{table} WHERE id=1 AND value=42"
